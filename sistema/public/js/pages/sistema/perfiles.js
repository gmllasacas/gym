...
jQuery(function () {
                ...
                switch (response.data[i]['estado']) {
                  case '2':
                    response.data[i]['acciones']+=
                                    '    <button class="btn btn-xs btn-success cambiadatoregistro" data-toggle="tooltip" data-placement="top" title="Activar" data-id="' + response.data[i]['id'] + '" data-table="base_perfil" data-valor="1" data-campo="estado">' +
                                    '        <i class="fa fa-check"></i>'+
                                    '    </button>';
                    break;
                  case '1':
                    response.data[i]['acciones']+=
                                    '    <button class="btn btn-xs btn-info editarregistro" data-toggle="tooltip" data-placement="top" title="Editar" data-id="'+response.data[i]['id']+'" data-table="base_perfil">'+
                                    '        <i class="fa fa-edit"></i>'+
                                    '    </button>'+
                                    '    <button class="btn btn-xs btn-warning cambiadatoregistro" data-toggle="tooltip" data-placement="top" title="Desactivar" data-id="' + response.data[i]['id'] + '" data-table="base_perfil" data-valor="2" data-campo="estado">' +
                                    '        <i class="fa fa-ban"></i>'+
                                    '    </button>';
                    break;
                  default:
      ...
      jQuery.ajax({
        ...
        success: function(response) {
          ...
          if(response.status=='200'){
            ...
            jQuery.each(response.registro.permisos, function(index, item) {
              jQuery.each(item, function(index_inner, item_inner) {
                jQuery('.submenu[data-id="'+item_inner.menu+'"]').trigger('click');
              });
            });
    ...
    jQuery('body').on('click', registroform + ' .menu', function() {
      if (this.checked) {
        $(this).closest('li.parent').find('.submenu').each(function () {
    ...
    jQuery('body').on('click', registroform + ' .submenu', function() {
      var total = $(this).closest('li.parent').find('.submenu').length;
      var count = 0;
      ...
      if(total == count) {
        $(this).closest('li.parent').find('.menu').prop('checked', true);
      } else {
        $(this).closest('li.parent').find('.menu').prop('checked', false);
      }
    });
});
