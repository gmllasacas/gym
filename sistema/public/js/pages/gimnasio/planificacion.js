...
jQuery(function () {
          ...
          switch (response.status) {
            case 200:
              for ( var i=0, ien=response.data.length ; i<ien ; i++ ) {
                ...
                switch (response.data[i]['estado']) {
                  case '2':
                    response.data[i]['acciones']+=
                                    '    <button class="btn btn-xs btn-success cambiadatoregistro" data-toggle="tooltip" data-placement="top" title="Activar" data-id="' + response.data[i]['id'] + '" data-table="proceso_curso" data-valor="1" data-campo="estado">' +
                                    '        <i class="fa fa-check"></i>'+
                                    '    </button>';
                    break;
                  case '1':
                    response.data[i]['acciones']+=
                                    '    <button class="btn btn-xs btn-info editarregistro" data-toggle="tooltip" data-placement="top" title="Editar" data-id="'+response.data[i]['id']+'" data-table="proceso_curso">'+
                                    '        <i class="fa fa-edit"></i>'+
                                    '    </button>'+
                                    '    <button class="btn btn-xs btn-warning cambiadatoregistro" data-toggle="tooltip" data-placement="top" title="Desactivar" data-id="' + response.data[i]['id'] + '" data-table="proceso_curso" data-valor="2" data-campo="estado">' +
                                    '        <i class="fa fa-ban"></i>'+
                                    '    </button>';
                    break;
                  default:
                ...
                response.data[i]['acciones'] += '<button class="btn btn-xs btn-danger cambiadatoregistro" data-toggle="tooltip" data-placement="top" title="Eliminar" data-id="' + response.data[i]['id'] + '" data-table="proceso_curso" data-valor="5" data-campo="estado">' +
    ...
    jQuery('body').on('click', '#editarplanificacion', function() {
      jQuery(registroform+' [name=planificacion]').trigger('click');
    });
      ...
      jQuery.ajax({
        ...
        success: function(response) {
          ...
          if(response.status=='200'){
            ...
            jQuery(registroform+' [name=codigo]').prop('disabled',true);
            jQuery(registromodal).modal('toggle');
          }
        }
      });
